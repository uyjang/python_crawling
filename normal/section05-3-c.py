...
# Login 정보(개발자 도구)에서 formdata부분에 있는 것을 확인 후 그대로 만들은 것
login_info = {
...
with req.session() as s:
    # Request(로그인 시도)
    res = s.post('https://auth.danawa.com/login', login_info, headers=headers) # 포스트로 요청 할 url적고 formdata에 들어갈 로그인인포 적고 헤더정보가 들어간 헤더스를 넣으면 됨
    ...
    # 본문 수신 데이터 확인
    # print(res.content.decode('UTF-8')) # content는 본문데이터를 가져오겠다 라는 뜻
