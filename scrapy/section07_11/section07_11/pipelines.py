...
class TestSpiderPipeline(object):
    ...
    # 초기화 메소드
    def __init__(self): # 클래스가 초기화 될 때 최초로 실행하는 것
        # 엑셀 처리 선언
    ...
    def process_item(self, item, spider):
        if int(item.get('rank_num')) < 41:
            item['is_pass'] = True
            # 엑셀 저장
            self.worksheet.write('A%s' % self.rowcount, item.get('rank_num')) # items.py는 딕셔너리 형태이므로 get메소드를 사용, get메소드는 벨류값이 없을 시 none으로 나오지만 item['rank_num']이렇게 키의 벨류값에 바로 접근하면 값이 없을 시 error발생
            ...
            # CSV 저장
            self.csv_writer.writerow(item) # items.py는 딕셔너리 형태이므로 
